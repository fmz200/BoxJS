...
function getSystemCfgs() {
  ...
  return {
    ...
    chavy: { id: 'ChavyLeung', icon: 'https://avatars3.githubusercontent.com/u/29748519', repo: 'https://github.com/fmz200/BoxJS' },
    ...
    boxjs: { id: 'BoxJs', show: false, icon: 'https://raw.githubusercontent.com/Orz-3/mini/master/Color/box.png', icons: ['https://raw.githubusercontent.com/Orz-3/mini/master/Alpha/box.png', 'https://raw.githubusercontent.com/Orz-3/mini/master/Color/box.png'], repo: 'https://github.com/fmz200/BoxJS' },
  ...
  const sysapps = [
    {
      ...
      icons: [
        'https://raw.githubusercontent.com/fmz200/BoxJS/master/box/icons/BoxSetting.mini.png',
        'https://raw.githubusercontent.com/fmz200/BoxJS/master/box/icons/BoxSetting.png'
      ]
    },
    {
      id: 'BoxSwitcher',
...
/**
 * 订阅 URL 归一化 (预留, 当前未接入唯一性校验)
 *
 * 将不同 GitHub 加速链路/镜像站点映射到同一标识, 供未来唯一性校验做二级判断。
 * 当前版本不影响任何调用链路, 仅作为独立工具函数保留。
 *
 * 示例:
 ...
 *   https://ghproxy.com/https://raw.githubusercontent.com/owner/repo/branch/...
 *   https://github.com/owner/repo/raw/branch/path/file.json
 *   → github:owner/repo/branch
 *
 * TODO: 在 apiAddAppSub / apiAddAppSubRaw 中接入唯一性校验时启用此函数
 */
function normalizeSubUrl(url) {
  ...
  // 去除缓存时间戳等查询参数
  u = u.replace(/[?&]_=\d+/g, '').replace(/[?#].*$/, '')

  // 常见 GitHub 加速前缀, 剥掉后继续解析内嵌 GitHub URL
  const proxyPrefixes = [
    /^https?:\/\/(?:ghproxy|gh-proxy|mirror\.ghproxy|ghfast|gh\.proxy|ghps)\.(?:com|net|top|org)\//i,
    /^https?:\/\/ghproxy\.net\//i
  ]
  for (const re of proxyPrefixes) {
    if (re.test(u)) {
      u = u.replace(re, '')
      break
    }
  ...
  // 若剥掉前缀后仍是加速站链接, 继续剥 (如 ghproxy 内嵌 ghproxy)
  for (let i = 0; i < 3; i++) {
    const again = proxyPrefixes.find((re) => re.test(u))
    if (!again) break
    u = u.replace(again, '')
  }
  ...
  const rawMatch = u.match(/^https?:\/\/raw\.githubusercontent\.com\/([^/]+)\/([^/]+)\/([^/]+)\/?/i)
  if (rawMatch) return `github:${rawMatch[1]}/${rawMatch[2]}/${rawMatch[3]}`

  // 匹配 github.com 上的 raw/ blob/ 等页面路径
  //   github.com/owner/repo/raw/branch/path
  const githubRawMatch = u.match(/^https?:\/\/github\.com\/([^/]+)\/([^/]+)\/raw\/([^/]+)\/?/i)
  if (githubRawMatch) return `github:${githubRawMatch[1]}/${githubRawMatch[2]}/${githubRawMatch[3]}`

  // 匹配 jsDelivr CDN
  //   cdn.jsdelivr.net/gh/owner/repo@branch/path
  const jsdelivrMatch = u.match(/^https?:\/\/cdn\.jsdelivr\.net\/gh\/([^/]+)\/([^/]+)@([^/]+)\/?/i)
  if (jsdelivrMatch) return `github:${jsdelivrMatch[1]}/${jsdelivrMatch[2]}/${jsdelivrMatch[3]}`

  // 匹配 fastly.jsdelivr.net 等镜像
  const fastlyMatch = u.match(/^https?:\/\/(?:fastly|gcore)\.jsdelivr\.net\/gh\/([^/]+)\/([^/]+)@([^/]+)\/?/i)
  if (fastlyMatch) return `github:${fastlyMatch[1]}/${fastlyMatch[2]}/${fastlyMatch[3]}`

  // 无法识别时返回空, 表示"不参与归一化判定"
  return ''
}
