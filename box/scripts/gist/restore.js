...
(async () => {
  ...
  const remoteFiles = boxjsdata && boxjsdata.files ? boxjsdata.files : {};
  if ($.backupType.indexOf("extraDatas") !== -1 && remoteFiles["extraDatas.json"]) {
    const fileUri = remoteFiles["extraDatas.json"].raw_url;
    const content = await getBackGist(fileUri);
    if (content) {
      let count = 0;
      Object.keys(content || {}).forEach((key) => {
        $.setdata(content[key], key);
        count++;
      });
      $.msg += `非订阅数据：extraDatas.json（${count} 项）\n`;
      $.info(`非订阅数据：extraDatas.json（${count} 项）恢复成功`);
    } else {
      $.msg += `非订阅数据：extraDatas.json 未找到备份，请先备份\n`;
      $.info(`非订阅数据：extraDatas.json 未找到备份，请先备份`);
    }
  }
