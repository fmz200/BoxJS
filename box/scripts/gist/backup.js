...
$.backupType =
  $.read("backup_type") || [...Object.keys(cacheArr), "datas", "extraDatas"].join(",");
...
(async () => {
  ...
  for (const cacheArrKey in cacheArr) {
    const enabled =
      $.backupType.indexOf(cacheArrKey) !== -1 ||
      (cacheArrKey === "globalbaks" && $.backupType.indexOf("backups") !== -1);
    if (!enabled) continue;
  ...
  if ($.backupType.indexOf("extraDatas") !== -1) {
    const extraDatas = getExtraDatas();
    files["extraDatas.json"] = { content: JSON.stringify(extraDatas) };
    $.msg += `非订阅数据：extraDatas.json（${Object.keys(extraDatas).length} 项）\n`;
  }
  ...
  const sysapps = [
    {
      ...
      scripts: [
        {
          name: '抹掉：所有缓存',
          script: 'https://raw.githubusercontent.com/fmz200/BoxJS/refs/heads/master/box/scripts/boxjs.revert.caches.js',
        },
        {
          name: '抹掉：收藏应用',
          script: 'https://raw.githubusercontent.com/fmz200/BoxJS/refs/heads/master/box/scripts/boxjs.revert.usercfgs.favapps.js',
        },
        {
          name: '抹掉：用户偏好',
          script: 'https://raw.githubusercontent.com/fmz200/BoxJS/refs/heads/master/box/scripts/boxjs.revert.usercfgs.js',
        },
        {
          name: '抹掉：所有会话',
          script: 'https://raw.githubusercontent.com/fmz200/BoxJS/refs/heads/master/box/scripts/boxjs.revert.usercfgs.sessions.js',
        },
        {
          name: '抹掉：所有备份',
          script: 'https://raw.githubusercontent.com/fmz200/BoxJS/refs/heads/master/box/scripts/boxjs.revert.baks.js',
        },
        {
          name: '抹掉：BoxJs (注意备份)',
          script: 'https://raw.githubusercontent.com/fmz200/BoxJS/refs/heads/master/box/scripts/boxjs.revert.boxjs.js',
        },
      ],
      author: '@chavyleung',
      repo: 'https://github.com/chavyleung/scripts/blob/master/box/switcher/box.switcher.js',
      icons: [
        'https://raw.githubusercontent.com/fmz200/BoxJS/refs/heads/master/box/icons/BoxSetting.mini.png',
        'https://raw.githubusercontent.com/fmz200/BoxJS/refs/heads/master/box/icons/BoxSetting.png',
      ],
    },
    {
      id: 'BoxSwitcher',
      ...
      repo: 'https://github.com/chavyleung/scripts/blob/master/box/switcher/box.switcher.js',
      icons: [
        'https://raw.githubusercontent.com/fmz200/BoxJS/refs/heads/master/box/icons/BoxSwitcher.mini.png',
        'https://raw.githubusercontent.com/fmz200/BoxJS/refs/heads/master/box/icons/BoxSwitcher.png',
      ],
      script: 'https://raw.githubusercontent.com/fmz200/BoxJS/refs/heads/master/box/switcher/box.switcher.js',
    },
  ];
  return sysapps;
}
...
function getAppKeys(app) {
  const keys = [];
  if (app && Array.isArray(app.keys)) {
    app.keys.forEach((key) => keys.push(key));
  }
  if (app && Array.isArray(app.settings)) {
    app.settings.forEach((setting) => {
      if (setting && setting.id) keys.push(setting.id);
    });
  }
  return keys;
}
...
function getSubscribedKeys() {
  ...
  getSystemApps().forEach((app) => {
    getAppKeys(app).forEach((k) => subscribedKeys.add(k));
  });
...
function getExtraDatas() {
  const subscribedKeys = getSubscribedKeys();
  const knownKeys = (() => {
    try {
      const raw = $.getval("chavy_boxjs_known_keys");
  ...
  const usercfgs = getUserCfgs();
  const includeList = Array.isArray(usercfgs.gist_extra_include) ? usercfgs.gist_extra_include : [];
  const includeSaved = !!usercfgs.gist_extra_saved;
  const extraData = {};
  knownKeys.forEach((key) => {
    if (
      !key ||
      subscribedKeys.has(key) ||
      /^(chavy_boxjs_|#|@?gist\.)/.test(key) ||
      key === "gist"
    ) {
      return;
    }
    if (includeSaved && !includeList.includes(key)) return;
    const val = $.getdata(key);
